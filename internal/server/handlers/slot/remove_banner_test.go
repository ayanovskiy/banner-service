...
func TestRemoveBannerFromSlot(t *testing.T) {
	t.Run("корректное удаление баннера из слота", func(t *testing.T) {
		...
		gomock.InOrder(
			...
			s.EXPECT().RemoveBannerFromSlot(uint(1), uint(1)).Return(nil),
		)
	...
	t.Run("перехват ошибки при удалении баннера", func(t *testing.T) {
		ctrl := gomock.NewController(t)
